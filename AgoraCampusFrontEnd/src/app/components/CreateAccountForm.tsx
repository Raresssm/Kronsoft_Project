...
export function CreateAccountForm() {
  ...
  const { registerWithCredentials, error: authError } = useAuth();
      ...
      <form className="mt-8 space-y-5" onSubmit={(event) => void handleSubmit(event)}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <label className="block">
            ...
            <div className="flex items-center gap-2 border-b border-white/20 pb-2 text-white/50 transition focus-within:border-white">
              <input
                type="text"
        ...
        {error && <p className="text-center text-xs text-red-400">{error || authError}</p>}
